...
public readonly partial struct EventDataType
{
    private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
    {
        ...
        int propertyCount = 0;
        bool foundEventIdentifier = false;
        ...
        foreach (JsonObjectProperty property in this.EnumerateObject())
        {
            if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
            {
                ...
                if ((this.HasJsonElementBacking && property.NameEquals(EventIdentifierUtf8JsonPropertyName)) || (!this.HasJsonElementBacking && property.NameEquals(EventIdentifierJsonPropertyName)))
                {
                    foundEventIdentifier = true;
                }
        ...
        if (!foundEventIdentifier)
        {
            if (level >= ValidationLevel.Detailed)
            {
                result = result.WithResult(isValid: false, $"6.5.3. required - required property \"eventIdentifier\" not present.");
            }
