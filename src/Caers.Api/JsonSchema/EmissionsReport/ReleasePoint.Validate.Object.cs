...
public readonly partial struct ReleasePoint
{
    private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
    {
        ...
        int propertyCount = 0;
        bool foundReleasePointTypeCode = false;
        foreach (JsonObjectProperty property in this.EnumerateObject())
        {
            if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
            {
                ...
                if ((this.HasJsonElementBacking && property.NameEquals(ReleasePointTypeCodeUtf8JsonPropertyName)) || (!this.HasJsonElementBacking && property.NameEquals(ReleasePointTypeCodeJsonPropertyName)))
                {
                    foundReleasePointTypeCode = true;
                }
            }
        ...
        if (!foundReleasePointTypeCode)
        {
            if (level >= ValidationLevel.Detailed)
            {
                result = result.WithResult(isValid: false, $"6.5.3. required - required property \"releasePointTypeCode\" not present.");
            }
