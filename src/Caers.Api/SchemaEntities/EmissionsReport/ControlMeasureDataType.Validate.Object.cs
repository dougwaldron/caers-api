...
public readonly partial struct ControlMeasureDataType
{
    private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
    {
        ...
        int propertyCount = 0;
        bool foundControlMeasureCode = false;
        foreach (JsonObjectProperty property in this.EnumerateObject())
        {
            if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
            {
                ...
                if ((this.HasJsonElementBacking && property.NameEquals(ControlMeasureCodeUtf8JsonPropertyName)) || (!this.HasJsonElementBacking && property.NameEquals(ControlMeasureCodeJsonPropertyName)))
                {
                    foundControlMeasureCode = true;
                }
            }
        ...
        if (!foundControlMeasureCode)
        {
            if (level >= ValidationLevel.Detailed)
            {
                result = result.WithResult(isValid: false, $"6.5.3. required - required property \"controlMeasureCode\" not present.");
            }
