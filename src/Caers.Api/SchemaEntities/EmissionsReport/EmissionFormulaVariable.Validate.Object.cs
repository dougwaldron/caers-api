...
public readonly partial struct EmissionFormulaVariable
{
    private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
    {
        ...
        int propertyCount = 0;
        bool foundEmissionFormulaVariableCode = false;
        ...
        foreach (JsonObjectProperty property in this.EnumerateObject())
        {
            if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
            {
                ...
                if ((this.HasJsonElementBacking && property.NameEquals(EmissionFormulaVariableCodeUtf8JsonPropertyName)) || (!this.HasJsonElementBacking && property.NameEquals(EmissionFormulaVariableCodeJsonPropertyName)))
                {
                    foundEmissionFormulaVariableCode = true;
                }
        ...
        if (!foundEmissionFormulaVariableCode)
        {
            if (level >= ValidationLevel.Detailed)
            {
                result = result.WithResult(isValid: false, $"6.5.3. required - required property \"emissionFormulaVariableCode\" not present.");
            }
