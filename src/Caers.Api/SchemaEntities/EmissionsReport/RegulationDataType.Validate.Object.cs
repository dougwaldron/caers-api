...
public readonly partial struct RegulationDataType
{
    private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
    {
        ...
        int propertyCount = 0;
        bool foundRegulatoryCode = false;
        foreach (JsonObjectProperty property in this.EnumerateObject())
        {
            if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
            {
                ...
                if ((this.HasJsonElementBacking && property.NameEquals(RegulatoryCodeUtf8JsonPropertyName)) || (!this.HasJsonElementBacking && property.NameEquals(RegulatoryCodeJsonPropertyName)))
                {
                    foundRegulatoryCode = true;
                }
            }
        ...
        if (!foundRegulatoryCode)
        {
            if (level >= ValidationLevel.Detailed)
            {
                result = result.WithResult(isValid: false, $"6.5.3. required - required property \"regulatoryCode\" not present.");
            }
