...
public readonly partial struct EventLocationDataType
{
    private ValidationContext ValidateObject(JsonValueKind valueKind, in ValidationContext validationContext, ValidationLevel level)
    {
        ...
        int propertyCount = 0;
        bool foundEventEmissionsProcess = false;
        foreach (JsonObjectProperty property in this.EnumerateObject())
        {
            if (__TryGetCorvusLocalPropertiesValidator(property, this.HasJsonElementBacking, out ObjectPropertyValidator? propertyValidator))
            {
                ...
                if ((this.HasJsonElementBacking && property.NameEquals(EventEmissionsProcessUtf8JsonPropertyName)) || (!this.HasJsonElementBacking && property.NameEquals(EventEmissionsProcessJsonPropertyName)))
                {
                    foundEventEmissionsProcess = true;
                }
            }
        ...
        if (!foundEventEmissionsProcess)
        {
            if (level >= ValidationLevel.Detailed)
            {
                result = result.WithResult(isValid: false, $"6.5.3. required - required property \"eventEmissionsProcess\" not present.");
            }
